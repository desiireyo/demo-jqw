...
<form id="fmsetzone">
    <table class="setzone-table">
        <tr>
            <td>รหัสการตลาด</td>
            <td><input type="text" id="<?php echo $menucod ?>zonecode"></td>
        </tr>
        <tr>
            <td>คำอธิบาย</td>
            <td><input type="text" id="<?php echo $menucod ?>zonedesc"></td>
        </tr>
    </table>
</form>
<br />
...
<script type="text/javascript">
    $(document).ready(function () {
    	...
    	$("#"+menucod+"zonecode").jqxInput({ height: 23, width: 121, theme: theme, minLength: 1, maxLength: 10 });
        ...
        $('#fmsetzone').jqxValidator({
            ...
            rules:  [
                        ...
                        { input: '#'+menucod+'zonecode', message: 'ระบุรหัสการตลาดซ้ำ!', action: 'keyup, blur', rule: function (input, commit) {
                                var zonecode = $("#"+menucod+"zonecode").val();
                                if (zonecode !== '') {
                                    $.ajax({
                                        url: "setzone/validate",
                                        type: 'get',
